...
const STATE = {
  ...
  zoomRatio:      parseFloat(localStorage.getItem('reentry_zoomRatio')) || 1,  // user's zoom preference, carried across slides
  programmaticCameraMove: false,  // true while a tween/snap we triggered is in flight (ignore for zoom-ratio tracking)
  lastCamDefault: { pos: [0, 12, 20], target: [0, 0, 0] },  // unadjusted default cam for whatever is currently shown
  persistent: {
    earthMesh:    null,
...
// ── Annotation state ───────────────────────────────────────────────────────
const ANNOT_COLORS = ['#FF4444', '#44FF88', '#4499FF', '#FFEE44', '#FF88FF'];
const ANNOT = {
  mode:     'off',  // 'off' | 'laser' | 'pen'
  colorIdx: 0,
  drawing:  false,
  lastX:    0,
  lastY:    0,
  strokes:  [],     // persistent: [{color, points:[{x,y}]}]
  laser:    [],     // [{x0,y0,x1,y1,t,color}] — auto-fades
  rafId:    null,
};

// ── Three.js globals ───────────────────────────────────────────────────────
let renderer, camera, scene, controls, css2d, clock;
let annotCanvas, annotCtx, annotToolbar, ANNOT_API;
...
function initScene() {
  ...
  // Track the user's zoom preference (distance ratio vs. each view's default)
  // so it carries over across slide/substep transitions instead of resetting.
  controls.addEventListener('change', () => {
    if (STATE.programmaticCameraMove) return;
    const defTarget = new THREE.Vector3(...STATE.lastCamDefault.target);
    const defDist   = new THREE.Vector3(...STATE.lastCamDefault.pos).sub(defTarget).length();
    if (defDist < 1e-6) return;
    const dist = camera.position.distanceTo(controls.target);
    STATE.zoomRatio = THREE.MathUtils.clamp(dist / defDist, 0.3, 3);
    localStorage.setItem('reentry_zoomRatio', STATE.zoomRatio);
  });
...
function loadAssets() {
  return new Promise((resolve) => {
    ...
    new THREE.TextureLoader().load(window._EARTH_SRC, (tex) => {
      ...
      const CDN_EARTH = 'https://cdn.jsdelivr.net/gh/mrdoob/three.js@r164/examples/textures/planets/earth_atmos_2048.jpg';
...
// so it reads as a soft halo rather than a flat tinted sphere. Uses the true
// per-fragment view vector (same pattern as _FRESNEL_FRAG below) rather than
// a fixed (0,0,1) approximation — that shortcut made the ring's apparent
// thickness balloon at long camera distances since it ignored perspective.
const _ATMO_VERT = `
...
const _ATMO_FRAG = `
  ...
  varying vec3 vViewPos;
  void main() {
    // BackSide faces have outward normals pointing away from the camera, so
    // dot(N, viewDir) is <= 0 here; add 1 to map silhouette(~0) -> bright(1)
    // and far-side-center(~-1) -> dark(0).
    float rim = clamp(1.0 + dot(normalize(vNormal), normalize(vViewPos)), 0.0, 1.0);
    float intensity = pow(rim, 8.0);
...
function buildEarth(texture) {
  ...
  scene.add(new THREE.Mesh(new THREE.SphereGeometry(EARTH_RADIUS * 1.02, 64, 64), atmoMat));
}
...
// Rescales a default cam position by the user's carried-over zoom ratio,
// keeping the same direction from target and clamped to controls' distance limits.
function applyZoomRatio(pos, target) {
  const t = new THREE.Vector3(...target);
  const dir = new THREE.Vector3(...pos).sub(t);
  const dist = dir.length();
  if (dist < 1e-6) return pos;
  const newDist = THREE.MathUtils.clamp(dist * STATE.zoomRatio, controls.minDistance, controls.maxDistance);
  dir.setLength(newDist);
  return t.add(dir).toArray();
}

function tweenCamera(toPos, toTarget, duration) {
  if (STATE.gsapTween) STATE.gsapTween.kill();
  STATE.lastCamDefault = { pos: toPos, target: toTarget };
  const adjustedPos = applyZoomRatio(toPos, toTarget);
  const fromPos    = camera.position.clone();
  const fromTarget = controls.target.clone();
  const proxy = { t: 0 };
  STATE.programmaticCameraMove = true;
  STATE.gsapTween = gsap.to(proxy, {
    t: 1, duration, ease: 'power2.inOut',
    onUpdate() {
      camera.position.lerpVectors(fromPos,
        new THREE.Vector3(...adjustedPos), proxy.t);
      controls.target.lerpVectors(fromTarget,
        new THREE.Vector3(...toTarget), proxy.t);
      controls.update();
    },
    onComplete() {
      STATE.programmaticCameraMove = false;
    },
  });
}
...
// Fresnel rim shader on the cone material — same color, same geometry, no duplicates.
// The rim brightens toward the silhouette edge and fades to the base color at center,
// giving a soft emissive highlight without any extra objects.
const _FRESNEL_VERT = `
  varying vec3 vNormal;
  varying vec3 vViewPos;
  void main() {
...
const _FRESNEL_FRAG = `
  ...
  varying vec3  vViewPos;
  void main() {
    float rim  = 1.0 - clamp(dot(normalize(vNormal), normalize(vViewPos)), 0.0, 1.0);
    float glow = pow(rim, 2.0) * glowStrength;
    gl_FragColor = vec4(min(baseColor * (1.0 + glow), vec3(1.0)), 1.0);
  }`;

function highlightArrow(arrow, hexColor) {
  if (!arrow || !arrow.cone) return;
  if (arrow.cone.material) arrow.cone.material.dispose();
  arrow.cone.material = new THREE.ShaderMaterial({
    uniforms: {
...
function restoreArrowMaterial(arrow, hexColor) {
  if (!arrow || !arrow.cone) return;
  if (arrow.cone.material) arrow.cone.material.dispose();
  arrow.cone.material = new THREE.MeshBasicMaterial({ color: hexColor, toneMapped: false });
}

function restoreAllArrowMaterials() {
  restoreArrowMaterial(STATE.persistent.gravArrow,   COLORS.grav);
  restoreArrowMaterial(STATE.persistent.dragArrow,   COLORS.drag);
  restoreArrowMaterial(STATE.persistent.liftArrow,   COLORS.lift);
  restoreArrowMaterial(STATE.persistent.thrustArrow, COLORS.thrust);
}
...
// Shrink #slide-body's font size just enough to eliminate vertical scrolling,
// never below the pre-bump baseline (0.72rem of the 21px root ≈ the original 15px).
const SLIDE_BODY_MAX_REM = 1.0;
const SLIDE_BODY_MIN_REM = 0.72;
function fitSlideBody() {
  const panel = document.getElementById('slide-body');
  if (!panel) return;
  let scale = SLIDE_BODY_MAX_REM;
  panel.style.fontSize = scale + 'rem';
  let guard = 0;
  while (panel.scrollHeight > panel.clientHeight && scale > SLIDE_BODY_MIN_REM && guard < 24) {
    scale = Math.max(SLIDE_BODY_MIN_REM, scale - 0.03);
    panel.style.fontSize = scale + 'rem';
    guard++;
  }
}
...
function buildECIChain(forceVec, origin, gen, baseDelay = 0.45) {
  ...
  const p1 = p0.clone().add(new THREE.Vector3(forceVec.x, 0, 0));        // ECI X step
  const p2 = p1.clone().add(new THREE.Vector3(0, 0, forceVec.z));        // ECI Ŷ step (Three.js Z)
  const p3 = p2.clone().add(new THREE.Vector3(0, forceVec.y, 0));        // ECI Ẑ step (Three.js Y)
  const defs = [
    { from: p0, to: p1, col: COLORS.eci.x, label: 'X̂' },
    { from: p1, to: p2, col: COLORS.eci.y, label: 'Ŷ' },
    { from: p2, to: p3, col: COLORS.eci.z, label: 'Ẑ' },
  ];
...
const SLIDES = [
  // ── 0: Introduction ────────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <ul>
        ...
        <li><strong>Scroll</strong> — zoom in/out (your zoom level carries over between slides)</li>
        <li><strong>0</strong> — reset zoom to this slide's default</li>
      </ul>
  ...
  // ── 1: ECI Frame ───────────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <p>All subsequent frames — OX₁Y₁Z₁, OX₂Y₂Z₂, OX″Y″Z″ — are defined by rotations
      relative to OXYZ. The DCMs on slide&nbsp;14 give the full transformation chain.</p>`,
  ...
  // ── 2: ECEF Frame ──────────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <span class="chip chip-ecef">amber OX₁Y₁Z₁ axes</span> are offset from the
      <span class="chip chip-eci">white OXYZ axes</span> by the current Earth rotation
      angle \\(\\omega_\\oplus\\Delta t\\). As Earth spins, this offset grows continuously.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-omega">ω Earth's rotation rate</span></div>`,
  ...
  // ── 3: Vehicle-Pointing Frame ──────────────────────────────────────────
  {
    ...
    html: `
      ...
      <div class="eq-block">
        ...
        \\[\\left[\\hat{e}_2\\right] = R_{y_2}(-\\phi)\\,R_{z_1}(\\theta)\\left[\\hat{e}_1\\right]\\]
      </div>
      <h3>RST shorthand used in this presentation</h3>
      <p>Later slides write this frame as <strong>RST</strong> — a compact alias:</p>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.5rem;margin:0.5rem 0;font-size:0.85rem;">
        <div style="padding:0.5rem;background:#07101e;border:1px solid #CC44FF;border-radius:6px;text-align:center;">
          <span style="color:#CC44FF;font-weight:700">R̂</span><br>
          <span style="color:#888;font-size:0.75rem">= x₂ = Radial</span>
        </div>
        <div style="padding:0.5rem;background:#07101e;border:1px solid #FF44CC;border-radius:6px;text-align:center;">
          <span style="color:#FF44CC;font-weight:700">Ŝ</span><br>
          <span style="color:#888;font-size:0.75rem">= y₂ = East</span>
        </div>
        <div style="padding:0.5rem;background:#07101e;border:1px solid #44FFCC;border-radius:6px;text-align:center;">
          <span style="color:#44FFCC;font-weight:700">T̂</span><br>
          <span style="color:#888;font-size:0.75rem">= z₂ = Cross-track</span>
        </div>
      </div>
      <p style="font-size:0.8rem;color:#6a90b0;">
        Any time you see a force vector with subscript <em>RST</em>, its three
        components are along R̂, Ŝ, T̂ = x₂, y₂, z₂ of the Vehicle-Pointing Frame.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-theta">θ longitude rotation angle</span></div>`,
  ...
  // ── 4: Velocity-Referenced Frame ───────────────────────────────────────
  {
    ...
    html: `
      ...
      <p style="font-size:0.82rem;color:#6a90b0;">
        <span style="color:#FF44CC">\\(\\psi\\)</span> is the <strong>heading angle</strong> — the compass
        direction of horizontal travel — and <span style="color:#FFEE77">\\(\\gamma\\)</span> is the
        <strong>flight-path angle</strong> — how steeply the vehicle is climbing or descending. Both are
        defined precisely in the next few slides; for now, just the two rotations that build the VRF.</p>
      <div class="eq-block">
      ...
      then rotated back to RST via the transpose of this rotation chain.</p>
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`,
    camera: { pos: [3, 3, 7], target: [1.17, 0.72, 0.72], dur: 1.0 },
    enter() {
      STATE.persistent.orbitLine.visible = true;
  ...
  // ── 5: Longitude & Latitude ────────────────────────────────────────────
  {
    ...
    html: `
      ...
      to decompose <em>v</em> into its \\(\\lambda\\) and \\(\\phi\\) rate components.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span></div>`,
    camera: { pos: [0, -4.5, 4.5], target: [0, 0, 0], dur: 0 },
    enter() {
      ...
      tweenCamera([0, -4.5, 4.5], [0, 0, 0], 1.0);
    },
    exit() {
      const s = getSpacecraftState(0.72);
      if (STATE.spacecraft) {
  ...
  // ── 6: Velocity Decomposition — Radial & Horizontal ────────────────────
  {
    ...
    html: `
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span></div>`,
  ...
  // ── 7: Velocity Decomposition — Heading Split ──────────────────────────
  {
    ...
    html: `
      ...
      <p style="font-size:0.82rem;color:#6a90b0;margin-top:0.4rem;">
        ...
        \\(\\textcolor{#FF44CC}{\\lambda}\\) degrees near the equator than near the poles.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`,
    camera: { pos: [3, 5, 7], target: [0, 0, 0], dur: 0 },
    enter() {
      clearSlideObjects();
      STATE.persistent.orbitLine.visible = true;
      ...
      // ── Horizontal speed component (cyan) ──
      const cg = new THREE.Group();
      cg.position.copy(s.pos);
      cg.add(new THREE.ArrowHelper(hDir, new THREE.Vector3(), hLen, 0x00DDFF, 0.07, 0.035));
      cg.scale.set(0.001, 0.001, 0.001);
      ...
      // ── Ground track: latitude circle (φ) & longitude meridian (λ) through the vehicle's nadir ──
      const phiAngle = Math.asin(R.y);
      const azimuth  = Math.atan2(R.z, R.x);
      const horizDir = new THREE.Vector3(R.x, 0, R.z).normalize();

      const latMat = new THREE.LineBasicMaterial({ color: 0x44FFEE, transparent: true, opacity: 0 });
      const latCircle = makeArc(
        new THREE.Vector3(0, EARTH_RADIUS * R.y, 0), new THREE.Vector3(0, 1, 0),
        EARTH_RADIUS * Math.cos(phiAngle) * 1.003, 0, Math.PI * 2, 0x44FFEE
      );
      latCircle.material = latMat;
      addSlideObj(latCircle);

      const lonMat = new THREE.LineBasicMaterial({ color: 0xFF44CC, transparent: true, opacity: 0 });
      const lonMeridian = makeArc(
        new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 1, 0).cross(horizDir).normalize(),
        EARTH_RADIUS * 1.003, 0, Math.PI * 2, 0xFF44CC
      );
      lonMeridian.material = lonMat;
      addSlideObj(lonMeridian);

      const nadirMat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0 });
      const nadirMarker = new THREE.Mesh(new THREE.SphereGeometry(0.018, 12, 12), nadirMat);
      nadirMarker.position.copy(R).multiplyScalar(EARTH_RADIUS * 1.004);
      addSlideObj(nadirMarker);

      const phiLabelAz  = azimuth + Math.PI / 2;
      const phiLabelPos = new THREE.Vector3(
        Math.cos(phiAngle) * Math.cos(phiLabelAz), Math.sin(phiAngle), Math.cos(phiAngle) * Math.sin(phiLabelAz)
      ).multiplyScalar(EARTH_RADIUS * 1.02);
      const lonLabelPhi = THREE.MathUtils.clamp(phiAngle + (phiAngle >= 0 ? 0.35 : -0.35), -1.4, 1.4);
      const lambdaLabelPos = new THREE.Vector3(
        Math.cos(lonLabelPhi) * Math.cos(azimuth), Math.sin(lonLabelPhi), Math.cos(lonLabelPhi) * Math.sin(azimuth)
      ).multiplyScalar(EARTH_RADIUS * 1.02);

      const enG = new THREE.Group();
      enG.add(makeFloatLabel('v', s.pos.clone().addScaledVector(vHat, vLen * 0.55).addScaledVector(R, 0.10), 0xFFD700));
      enG.add(makeFloatLabel('v cosγ', s.pos.clone().addScaledVector(hDir, hLen * 0.5).addScaledVector(R, -0.13), 0x00DDFF));
      ...
      enG.add(makeFloatLabel('φ', phiLabelPos, 0x44FFEE));
      enG.add(makeFloatLabel('λ', lambdaLabelPos, 0xFF44CC));
      enG.visible = false;
      addSlideObj(enG);
      ...
      gsap.delayedCall(CAM_DELAY, () => {
        ...
        gsap.to(vg.scale, { x:1,y:1,z:1, duration:0.55, ease:'back.out(1.3)' });
        gsap.to(cg.scale, { x:1,y:1,z:1, duration:0.75, delay:0.45, ease:'power2.out' });
        if (mg) gsap.to(mg.scale, { x:1,y:1,z:1, duration:0.75, delay:1.05, ease:'power2.out' });
        gsap.to(ng.scale,  { x:1,y:1,z:1, duration:0.60, delay:1.65, ease:'power2.out' });
        gsap.to(rbMat, { opacity:0.55, duration:0.35, delay:2.25 });
        gsap.to(psiArcMat, { opacity:0.90, duration:0.35, delay:2.25 });
        gsap.to(latMat,   { opacity:0.65, duration:0.5, delay:2.25 });
        gsap.to(lonMat,   { opacity:0.65, duration:0.5, delay:2.25 });
        gsap.to(nadirMat, { opacity:1.0,  duration:0.35, delay:2.25 });
        gsap.delayedCall(2.35, () => {
          if (STATE.slideGen !== gen1) return;
          enG.visible = true;
  ...
  // ── 8: Flight-Path Angle γ ─────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <div id="gamma-readout" style="margin-top:0.9rem;padding:0.7rem 1.1rem;background:#07162a;border:1px solid #1e3a5f;border-radius:8px;text-align:center;line-height:1.7;"></div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span></div>`,
    ...
    enter() {
      ...
      gammaLblEl.style.cssText = 'font-size:18px;font-weight:700;font-family:Segoe UI,system-ui,sans-serif;pointer-events:none;text-shadow:0 0 8px rgba(0,0,0,1);white-space:nowrap;';
      ...
      function updateHUD(g) {
        ...
        if (readout) {
          ...
          readout.innerHTML = `
            <div style="font-size:14px;letter-spacing:.1em;color:#3a6a9a;text-transform:uppercase;margin-bottom:2px">Live Flight-Path Angle</div>
            <div style="font-size:36px;font-weight:700;color:${color};letter-spacing:.02em">γ = ${sign}${deg.toFixed(1)}°</div>
            <div style="font-size:18px;color:${color}">${arrow}&nbsp;${label}</div>
          `;
        }
      }
  ...
  // ── 9: Heading Angle ψ ─────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <span style="color:#FF7799">\\(\\psi\\)</span> describes the direction of
      ...
      <p style="font-size:0.82rem;color:#6a90b0">
        ...
        <span style="color:#44FFEE">sin ψ</span>.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`,
    ...
    html: `
      ...
      <div style="display:flex;flex-direction:column;gap:0.45rem;margin:1rem 0;">
        <div style="display:flex;align-items:center;gap:0.4rem;font-size:0.8rem;">
          ...
          <span style="color:#2a5a8a;flex:1;padding:0.25rem 0.6rem;border:1px dashed #1a3a5a;border-radius:4px;text-align:center;">
            \\(R_z(\\omega_\\oplus\\Delta t)\\) &mdash; Earth spin
          </span>
          <span class="chip chip-ecef" style="white-space:nowrap">OX₁Y₁Z₁</span>
        </div>
        <div style="display:flex;align-items:center;gap:0.4rem;font-size:0.8rem;">
          <span class="chip chip-ecef" style="white-space:nowrap">OX₁Y₁Z₁</span>
          <span style="color:#2a5a8a;flex:1;padding:0.25rem 0.6rem;border:1px dashed #1a3a5a;border-radius:4px;text-align:center;">
            \\(R_{y_2}(-\\phi)\\,R_{z_1}(\\theta)\\) &mdash; Vehicle-Pointing
          </span>
          <span class="chip chip-rst" style="white-space:nowrap">OX₂Y₂Z₂</span>
        </div>
        <div style="display:flex;align-items:center;gap:0.4rem;font-size:0.8rem;">
          <span class="chip chip-rst" style="white-space:nowrap">OX₂Y₂Z₂</span>
          <span style="color:#2a5a8a;flex:1;padding:0.25rem 0.6rem;border:1px dashed #1a3a5a;border-radius:4px;text-align:center;">
            \\(R_{z'}(-\\gamma)\\,R_{x_2}(\\psi)\\) &mdash; Velocity-Referenced
          </span>
          <span class="chip chip-vrf" style="white-space:nowrap">OX″Y″Z″</span>
        </div>
      </div>
      <p style="font-size:0.82rem;color:#6a90b0;">Press <strong>Next →</strong> to step through each animated DCM.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ longitude rotation angle</span><span class="chip chip-omega">ω Earth's rotation rate</span></div>`,
  ...
  // ── 11: OXYZ → OX₁Y₁Z₁ — Earth Spin ──────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-omega">ω Earth's rotation rate</span></div>`);
    },
  ...
  // ── 12: OX₁Y₁Z₁ → OX₂Y₂Z₂ — Vehicle-Pointing ─────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-theta">θ longitude rotation angle</span></div>`);
    },
    exit() { STATE.suppressEarthUpdate = false; },
  },
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        <p style="font-size:0.82rem;color:#6a90b0;margin-top:0.5rem;">
          ê<sub>y″</sub> corresponds to x̂<sub>v</sub> in this presentation.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`);
    },
    exit() {},
  },
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        <p style="font-size:0.8rem;color:#6a90b0;margin-top:0.4rem;">
          ...
          To go OX″→OXYZ, transpose each matrix and multiply in reverse.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ longitude rotation angle</span></div>`);
    },
    exit() {},
  },
  ...
  // ── 15: Kinematic Equations (ṙ, φ̇, λ̇) ────────────────────────────────
  {
    ...
    html: `
      ...
      <p style="font-size:0.82rem;color:#6a90b0;margin-top:0.4rem;">
        ...
        — longitude circles shrink toward the poles.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`,
  ...
  // ── 17: Rotating Frame Conversion ───────────────────────────────────────
  {
    ...
    html: `
      ...
      direction along the orbit.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-omega">ω Earth's rotation rate</span></div>`,
  ...
  // ── 18: Gravitational Force ────────────────────────────────────────────
  {
    ...
    html: `
      ...
      descends closer to Earth.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-mu">μ gravitational parameter</span></div>`,
  ...
  // ── 19: Gravity — ECI Decomposition ────────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        </p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-mu">μ gravitational parameter</span></div>`);
  ...
  // ── 20: Drag Force ─────────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-rho">ρ atmospheric density</span></div>`,
  ...
  // ── 21: Drag — ECI Decomposition ───────────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        ${eciEqBlock(`\\vec{F}_D = \\underbrace{C_{E\\leftarrow R}(\\phi,\\theta)}_{\\text{RST}\\to\\text{ECI}}\\!\\begin{bmatrix}\\textcolor{#FF9944}{-D}\\sin\\textcolor{#FFEE77}{\\gamma}\\\\\\textcolor{#FF9944}{-D}\\cos\\textcolor{#FFEE77}{\\gamma}\\cos\\textcolor{#FF44CC}{\\psi}\\\\\\textcolor{#FF9944}{-D}\\cos\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}\\end{bmatrix}_{RST} = \\begin{bmatrix}F_{D,x}\\\\F_{D,y}\\\\F_{D,z}\\end{bmatrix}_{ECI}`)}
        <p style="font-size:0.8rem;color:#6a8aaa;">
          <span style="color:#FF3333">&#9632;</span> X̂ &ensp;
          <span style="color:#33FF33">&#9632;</span> Ŷ &ensp;
          <span style="color:#3399FF">&#9632;</span> Ẑ
        </p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ bank angle</span></div>`);
  ...
  // ── 22: Lift Force & Bank Angle ────────────────────────────────────────
  {
    ...
    html: `
      ...
      changing lift magnitude.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-theta">θ bank angle</span></div>`,
    camera: { pos: [1.83, 0.19, 0.19], target: [1.165, 0.722, 0.722], dur: 1.0 },
    enter() {
      STATE.persistent.orbitLine.visible = true;
      setFrameVisibility({ vrf: true, vel: true });
      ...
      // Camera: 1 unit aft of spacecraft along velocity, Earth below (R_hat = up)
      camera.up.copy(R);
      tweenCamera(s.pos.clone().addScaledVector(v_h, -1.0).toArray(), s.pos.toArray(), 1.0);
      const lift_hat = R.clone().addScaledVector(v_h, -R.dot(v_h)).normalize();
      const lat_hat  = new THREE.Vector3().crossVectors(v_h, lift_hat).normalize();

      // ŷᵥ shows where lift points at θ=0 (away from Earth, ⊥ velocity)
  ...
  // ── 23: Lift — ECI Decomposition ───────────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        ${eciEqBlock(`\\vec{F}_L = \\underbrace{C_{E\\leftarrow R}(\\phi,\\theta)}_{\\text{RST}\\to\\text{ECI}}\\!\\begin{bmatrix}\\textcolor{#FF6699}{-L}\\cos\\textcolor{#44FFFF}{\\theta}\\cos\\textcolor{#FFEE77}{\\gamma}\\\\\\textcolor{#FF6699}{L}(\\cos\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FFEE77}{\\gamma}\\cos\\textcolor{#FF44CC}{\\psi}+\\sin\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FF44CC}{\\psi})\\\\\\textcolor{#FF6699}{L}(\\cos\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}-\\sin\\textcolor{#44FFFF}{\\theta}\\cos\\textcolor{#FF44CC}{\\psi})\\end{bmatrix}_{RST} = \\begin{bmatrix}F_{L,x}\\\\F_{L,y}\\\\F_{L,z}\\end{bmatrix}_{ECI}`)}
        <p style="font-size:0.8rem;color:#6a8aaa;">
          <span style="color:#FF3333">&#9632;</span> X̂ &ensp;
          <span style="color:#33FF33">&#9632;</span> Ŷ &ensp;
          <span style="color:#3399FF">&#9632;</span> Ẑ
  ...
  // ── 24: Thrust Force ───────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-alpha">α thrust pitch angle</span><span class="chip chip-beta">β thrust bank angle</span></div>`,
    camera: { pos: [4, 3, 7], target: [0, 0, 0], dur: 0.8 },
    enter() {
      STATE.persistent.orbitLine.visible = true;
      setFrameVisibility({ vrf: true, vel: true });
  ...
  // ── 25: Thrust — ECI Decomposition ─────────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        ${eciEqBlock(`\\vec{F}_T = \\underbrace{C_{E\\leftarrow R}(\\phi,\\theta)}_{\\text{RST}\\to\\text{ECI}}\\!\\begin{bmatrix}\\textcolor{#FF4444}{T}\\sin\\textcolor{#FFEE77}{\\gamma}\\\\\\textcolor{#FF4444}{T}\\cos\\textcolor{#FFEE77}{\\gamma}\\cos\\textcolor{#FF44CC}{\\psi}\\\\\\textcolor{#FF4444}{T}\\cos\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}\\end{bmatrix}_{RST} = \\begin{bmatrix}F_{T,x}\\\\F_{T,y}\\\\F_{T,z}\\end{bmatrix}_{ECI}`)}
        <p style="font-size:0.8rem;color:#6a8aaa;">
          <span style="color:#FF3333">&#9632;</span> X̂ &ensp;
          <span style="color:#33FF33">&#9632;</span> Ŷ &ensp;
          <span style="color:#3399FF">&#9632;</span> Ẑ
  ...
  // ── 26: Newton's Law — All Forces in ECI ───────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        ready to integrate for \\(r(t),\\,v(t),\\,\\gamma(t),\\,\\psi(t)\\).</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`);
  ...
  // ── 27: F_net — ECI Components ──────────────────────────────────────────
  {
    ...
    enter() {
      ...
      setSlidePanel(`
        ...
        </p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ bank angle</span><span class="chip chip-mu">μ gravitational parameter</span></div>`);
  ...
  // ── 28: Force Assembly in OX₂Y₂Z₂ ─────────────────────────────────────
  {
    ...
    html: `
      <p>All forces are expressed in <span class="chip chip-rst">OX₂Y₂Z₂</span> so their
      components can be summed axis-by-axis. Each RST component will produce one scalar ODE.</p>
      <div class="eq-block">
        <div class="eq-label">Radial x₂ — inward gravity + drag/thrust + lift</div>
        \\[F_{x_2} = \\textcolor{#6699FF}{-\\tfrac{\\mu m}{r^2}}
          + (\\textcolor{#FF4444}{T}-\\textcolor{#FF9944}{D})\\sin\\textcolor{#FFEE77}{\\gamma}
          - \\textcolor{#FF6699}{L}\\cos\\textcolor{#44FFFF}{\\theta}\\cos\\textcolor{#FFEE77}{\\gamma}\\]
      </div>
      <div class="eq-block">
        <div class="eq-label">South y₂ — drag/thrust + lift meridional</div>
        \\[F_{y_2} = (\\textcolor{#FF4444}{T}-\\textcolor{#FF9944}{D})\\cos\\textcolor{#FFEE77}{\\gamma}\\cos\\textcolor{#FF44CC}{\\psi}
          + \\textcolor{#FF6699}{L}(\\cos\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FFEE77}{\\gamma}\\cos\\textcolor{#FF44CC}{\\psi}
          + \\sin\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FF44CC}{\\psi})\\]
      </div>
      <div class="eq-block">
        <div class="eq-label">East z₂ — drag/thrust + lift lateral</div>
        \\[F_{z_2} = (\\textcolor{#FF4444}{T}-\\textcolor{#FF9944}{D})\\cos\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}
          + \\textcolor{#FF6699}{L}(\\cos\\textcolor{#44FFFF}{\\theta}\\sin\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}
          - \\sin\\textcolor{#44FFFF}{\\theta}\\cos\\textcolor{#FF44CC}{\\psi})\\]
      </div>
      <div style="display:flex;gap:0.4rem;flex-wrap:wrap;margin-top:0.5rem;">
        <span class="chip chip-grav">Gravity</span>
        <span class="chip chip-drag">Drag</span>
        <span class="chip chip-lift">Lift</span>
        <span class="chip chip-thrust">Thrust</span>
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ bank angle</span><span class="chip chip-mu">μ gravitational parameter</span></div>`,
    camera: { pos: [4, 3, 7], target: [0, 0, 0], dur: 0.8 },
    enter() {
      clearSlideObjects();
      STATE.orbitT = 0.72;
  ...
  // ── 29: RST Projection → v̇, γ̇, ψ̇ ──────────────────────────────────────
  {
    ...
    html: `
      <p>From slide 17: \\(m\\ddot{\\vec{r}}_I = m\\ddot{\\vec{r}}_{\\text{rot}} +
      2m\\vec{\\omega}_\\oplus\\times\\dot{\\vec{r}}_{\\text{rot}} +
      m\\vec{\\omega}_\\oplus\\times(\\vec{\\omega}_\\oplus\\times\\vec{r})\\).
      Rearranging and projecting onto each RST axis:</p>
      <div class="eq-block">
        <div class="eq-label">x₂ (radial) → v̇</div>
        \\[m\\dot{v} = F_{x_2} + m\\omega_\\oplus^2 r\\cos\\phi
          (\\sin\\textcolor{#FFEE77}{\\gamma}\\cos\\phi - \\cos\\textcolor{#FFEE77}{\\gamma}\\sin\\phi\\cos\\textcolor{#FF44CC}{\\psi})\\]
      </div>
      <div class="eq-block">
        <div class="eq-label">y₂ (south) → γ̇</div>
        \\[mv\\dot{\\textcolor{#FFEE77}{\\gamma}} = F_{y_2}
          - m\\!\\left(g - \\tfrac{v^2}{r}\\right)\\!\\cos\\textcolor{#FFEE77}{\\gamma}
          + 2m\\omega_\\oplus v\\cos\\phi\\cos\\textcolor{#FF44CC}{\\psi}
          + m\\omega_\\oplus^2 r\\cos\\phi(\\cos\\textcolor{#FFEE77}{\\gamma}\\cos\\phi
          + \\sin\\textcolor{#FFEE77}{\\gamma}\\sin\\phi\\cos\\textcolor{#FF44CC}{\\psi})\\]
      </div>
      <div class="eq-block">
        <div class="eq-label">z₂ (east) → ψ̇</div>
        \\[mv\\cos\\textcolor{#FFEE77}{\\gamma}\\,\\dot{\\textcolor{#FF44CC}{\\psi}} = F_{z_2}
          + \\tfrac{mv^2\\cos^2\\!\\textcolor{#FFEE77}{\\gamma}\\sin\\textcolor{#FF44CC}{\\psi}\\tan\\phi}{r}
          - 2m\\omega_\\oplus v(\\tan\\textcolor{#FFEE77}{\\gamma}\\cos\\phi\\cos\\textcolor{#FF44CC}{\\psi}-\\sin\\phi)
          + \\tfrac{m\\omega_\\oplus^2 r\\sin\\phi\\cos\\phi\\sin\\textcolor{#FF44CC}{\\psi}}{\\cos\\textcolor{#FFEE77}{\\gamma}}\\]
      </div>
      <p style="font-size:0.82rem;color:#6a90b0;margin-top:0.4rem;">
        Dividing through by <em>m</em> and <em>mv</em>, <em>mv</em>cosγ respectively gives
        v̇, γ̇, ψ̇. Adding the three kinematic equations (ṙ, λ̇, φ̇) completes the
        six-state system on the next slide.</p><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-omega">ω Earth's rotation rate</span></div>`,
    camera: { pos: [4, 3, 7], target: [0, 0, 0], dur: 0.0 },
    enter() {
      clearSlideObjects();
      STATE.orbitT = 0.72;
      setFrameVisibility({ rst: true, vel: true });
  ...
  // ── 30: Complete 3-DOF EOM ─────────────────────────────────────────────
  {
    ...
    html: `
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span><span class="chip chip-theta">θ bank angle</span><span class="chip chip-omega">ω Earth's rotation rate</span><span class="chip chip-alpha">α thrust pitch angle</span></div>`,
  ...
  // ── 31: Summary ────────────────────────────────────────────────────────
  {
    ...
    html: `
      ...
      <p class="slide-credit">Created by Maj &ldquo;MOAB&rdquo; Olsen</p>
      <div style="margin-top:0.75rem;border-top:1px solid #0e2035;padding-top:0.55rem;">
        <div style="font-size:0.68rem;letter-spacing:.1em;color:#3a6a9a;text-transform:uppercase;margin-bottom:0.35rem;">Reference Frames</div>
        <div style="display:flex;gap:0.3rem;flex-wrap:wrap;margin-bottom:0.45rem;">
      ...
      </div><div class="var-legend"><div class="var-legend-title">Variables</div><span class="chip chip-phi">φ latitude</span><span class="chip chip-lambda">λ longitude</span><span class="chip chip-gamma">γ flight-path angle</span><span class="chip chip-psi">ψ heading angle</span></div>`,
...
function goToSlide(idx) {
  ...
  if (cam.dur > 0) {
    ...
    STATE.lastCamDefault = { pos: cam.pos, target: cam.target };
    STATE.programmaticCameraMove = true;
    camera.position.set(...applyZoomRatio(cam.pos, cam.target));
...
function initAnnotation() {
  ...
  function currentColor() { return ANNOT_COLORS[ANNOT.colorIdx]; }

  function setMode(m) {
    ANNOT.mode = m;
    renderer.domElement.style.cursor = m !== 'off' ? 'crosshair' : '';
    document.getElementById('annot-laser').classList.toggle('annot-on', m === 'laser');
    document.getElementById('annot-pen').classList.toggle('annot-on',   m === 'pen');
  }
  ...
  function drawGlowSeg(x0, y0, x1, y1, col, isLaser) {
    ...
    if (isLaser) {
      ...
      ctx.shadowBlur = 0; ctx.strokeStyle = '#ffffff';
      ctx.lineWidth = 1.5; ctx.globalAlpha = 0.9;
      ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); ctx.stroke();
    } else {
      ctx.shadowBlur = 8; ctx.shadowColor = col;
      ctx.strokeStyle = col; ctx.lineWidth = 2.5; ctx.globalAlpha = 1;
      ctx.beginPath(); ctx.moveTo(x0, y0); ctx.lineTo(x1, y1); ctx.stroke();
    }
    ctx.restore();
  }
  ...
  function redraw() {
    ...
    for (const s of ANNOT.strokes) {
      for (let i = 1; i < s.points.length; i++) {
        const a = s.points[i - 1], b = s.points[i];
        drawGlowSeg(a.x, a.y, b.x, b.y, s.color, false);
      }
    }

    const now  = performance.now();
    const FADE = 1400;
    ANNOT.laser = ANNOT.laser.filter(seg => now - seg.t < FADE);
    for (const seg of ANNOT.laser) {
      const age = (now - seg.t) / FADE;
      ctx.save();
      ctx.shadowBlur = 14; ctx.shadowColor = seg.color;
      ctx.strokeStyle = seg.color; ctx.lineWidth = 3;
      ctx.lineCap = 'round'; ctx.globalAlpha = (1 - age) * 0.9;
      ctx.beginPath(); ctx.moveTo(seg.x0, seg.y0); ctx.lineTo(seg.x1, seg.y1); ctx.stroke();
      ctx.shadowBlur = 0; ctx.strokeStyle = '#ffffff';
      ctx.lineWidth = 1.5; ctx.globalAlpha = (1 - age) * 0.85;
      ctx.beginPath(); ctx.moveTo(seg.x0, seg.y0); ctx.lineTo(seg.x1, seg.y1); ctx.stroke();
      ctx.restore();
    }

    if (ANNOT.laser.length > 0)
      ANNOT.rafId = requestAnimationFrame(redraw);
    else
      ANNOT.rafId = null;
  }

  function scheduleRedraw() {
    if (!ANNOT.rafId) ANNOT.rafId = requestAnimationFrame(redraw);
  }

  function clearAll() {
    ANNOT.strokes = []; ANNOT.laser = [];
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  function onDown(e) {
    ...
    ANNOT.drawing = true;
    const p = getPos(e);
    ANNOT.lastX = p.x; ANNOT.lastY = p.y;
    if (ANNOT.mode === 'pen') ANNOT.strokes.push({ color: currentColor(), points: [p] });
  }

  function onMove(e) {
    if (!ANNOT.drawing || e.pointerType === 'touch') return;
    e.stopImmediatePropagation();
    const p = getPos(e);
    if (ANNOT.mode === 'pen') {
      const s = ANNOT.strokes[ANNOT.strokes.length - 1];
      s.points.push(p);
      drawGlowSeg(ANNOT.lastX, ANNOT.lastY, p.x, p.y, s.color, false);
    } else if (ANNOT.mode === 'laser') {
      ANNOT.laser.push({ x0: ANNOT.lastX, y0: ANNOT.lastY, x1: p.x, y1: p.y,
                         t: performance.now(), color: currentColor() });
      scheduleRedraw();
    }
    ANNOT.lastX = p.x; ANNOT.lastY = p.y;
  }
  ...
  document.getElementById('annot-laser').addEventListener('click', () =>
    setMode(ANNOT.mode === 'laser' ? 'off' : 'laser'));
  document.getElementById('annot-pen').addEventListener('click', () =>
    setMode(ANNOT.mode === 'pen' ? 'off' : 'pen'));
  document.getElementById('annot-clear').addEventListener('click', clearAll);
  document.querySelectorAll('.annot-swatch').forEach(sw => {
    sw.addEventListener('click', () => {
      ANNOT.colorIdx = +sw.dataset.idx;
      document.querySelectorAll('.annot-swatch').forEach((s, i) =>
        s.classList.toggle('annot-active', i === ANNOT.colorIdx));
    });
  });
...
async function boot() {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'l' || e.key === 'L')
      ANNOT_API.setMode(ANNOT.mode === 'laser' ? 'off' : 'laser');
    if (e.key === 'p' || e.key === 'P')
      ANNOT_API.setMode(ANNOT.mode === 'pen' ? 'off' : 'pen');
    if (e.key === 'Escape' && ANNOT.mode !== 'off')
    ...
    if (e.key === '0') {
      STATE.zoomRatio = 1;
      localStorage.setItem('reentry_zoomRatio', 1);
      tweenCamera(STATE.lastCamDefault.pos, STATE.lastCamDefault.target, 0.6);
    }
...
// Expose navigation API so automation tools can drive the presentation
window.goToSlide  = goToSlide;
window.advanceNext = advanceNext;
window.STATE  = STATE;
